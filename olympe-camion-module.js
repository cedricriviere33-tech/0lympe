...
(function (global) {
  ...
  /* Requis d'un mode donné : [{leg, i}] — i est l'index dans day.req */
  function reqsOf(day, m) {
    var out = [];
    (day.req || []).forEach(function (r, i) { if (r && r.kind === m) out.push({ leg: r, i: i }); });
    return out;
  }

  /* Trajet courant : voyage 1..6 OU requis. La cible est mémorisée par mode,
     pour que la bulle Arrivée ne déplace pas le départ en cours. */
  function getLeg() {
    var day = gDay(), t = st.tgt[st.mode];
    if (t.type === 'req') {
      var r = (day.req || [])[t.i];
      if (r && r.kind === st.mode) return r;
      st.tgt[st.mode] = { type: 'voy', n: 1 };     /* requis disparu : repli */
      return day.voy[1][st.mode];
    }
    return day.voy[t.n][st.mode];
  }

  /* Libellé lisible du trajet courant d'un mode */
  function trajetLabel(m) {
    var t = st.tgt[m], base = (m === 'dep' ? 'Départ' : 'Arrivée');
    if (t.type !== 'req') return base + ' ' + t.n;
    var list = reqsOf(gDay(), m);
    for (var k = 0; k < list.length; k++) {
      if (list[k].i === t.i) return 'Requis ' + base.toLowerCase() + ' ' + (k + 1);
    }
    return 'Requis ' + base.toLowerCase();
  }

  /* Création d'un requis — même forme de leg qu'un voyage, marqué par kind,
     exactement ce qu'Hermès lit dans day.req[]. */
  function addRequis(m) {
    var day = gDay(), leg = emptyLeg();
    leg.kind = m;
    day.req.push(leg);
    st.tgt[m] = { type: 'req', i: day.req.length - 1 };
    st.lastOp = null;
    saveDB(); _lastRaw = ODB().getItem(DBK);
    resetScan(false);
    render();
    toast('+ ' + trajetLabel(m) + ' créé', 'ok');
    var c = $('ocm-sc-c'); if (c) { try { c.focus(); } catch (e) { } }
  }
  ...
  var st = {
    open: false,
    mode: 'dep',          /* 'dep' = départ (écran principal) | 'arr' = bulle */
    pop: false,           /* bulle Arrivée ouverte */
    tgt: {                /* trajet visé par mode : voyage 1..6 ou requis */
      dep: { type: 'voy', n: 1 },
      arr: { type: 'voy', n: 1 }
    },
    date: todayISO(),
    ...
    hist: false,          /* panneau Historique ouvert */
    histOpen: null,       /* journée dépliée */
    histConfirm: null     /* suppression armée, en attente du 2e clic */
  };
  ...
  var CSS = [
    ...
    /* ── BULLE ARRIVÉE ─────────────────────────────────────────────────── */
    '#ocm-pop{position:fixed;inset:0;z-index:100064;display:none;align-items:center;',
    'justify-content:center;padding:18px;background:rgba(4,38,22,.48);}',
    '#ocm-pop.on{display:flex;animation:ocmFade .22s var(--e);}',
    '@keyframes ocmFade{from{opacity:0}to{opacity:1}}',
    /* La bulle vit hors de #ocm-ov : elle redéclare le jeu de variables, sinon
       var(--card) / var(--line) ne résolvent plus et tout devient transparent. */
    '#ocm-pop .bulle{width:100%;max-width:1000px;max-height:92vh;display:flex;flex-direction:column;',
    'background:#F3FBF6;border-radius:22px;overflow:hidden;border:2px solid #009841;',
    'box-shadow:0 32px 80px rgba(0,60,30,.42);',
    'animation:ocmBulle .34s cubic-bezier(.34,1.5,.5,1) both;',
    "font-family:'DM Sans','Segoe UI',-apple-system,BlinkMacSystemFont,sans-serif;color:#083E24;",
    '--j:#FDDD09;--b:#0E4194;--v:#009841;--c:#00A8A8;--r:#E51932;',
    '--acc:#009841;--acc2:#00B44E;--line:rgba(0,152,65,.16);--muted:rgba(6,80,45,.62);',
    '--card:#FFFFFF;--sh:0 6px 20px rgba(0,152,65,.10);--sh2:0 16px 38px rgba(0,152,65,.20);',
    '--e:cubic-bezier(.22,.85,.28,1);--s:cubic-bezier(.34,1.5,.5,1);}',
    '#ocm-pop *{box-sizing:border-box;margin:0;}',
    '#ocm-pop button{font-family:inherit;cursor:pointer;border:0;}',
    '#ocm-pop .ocm-in{color:#083E24;}',
    '#ocm-pop .ocm-in:focus{box-shadow:0 0 0 5px rgba(0,152,65,.16);}',
    '#ocm-pop .ocm-sel{color:#083E24;}',
    '#ocm-pop .ocm-stat.ok{color:#047a37;}',
    '#ocm-pop .ocm-k .kt,#ocm-pop .ocm-k .kv{color:var(--kc);}',
    '@keyframes ocmBulle{from{transform:translateY(26px) scale(.94);opacity:0}to{transform:none;opacity:1}}',
    '#ocm-pop .bhead{display:flex;align-items:center;gap:12px;padding:14px 20px;flex-wrap:wrap;',
    'background:linear-gradient(100deg,#009841,#00B44E);color:#fff;position:relative;',
    'overflow:hidden;flex-shrink:0;}',
    '#ocm-pop .bhead::after{content:"";position:absolute;left:0;right:0;bottom:0;height:4px;',
    'background:repeating-linear-gradient(90deg,#0E4194 0 34px,#009841 34px 68px,#00A8A8 68px 102px,#E51932 102px 136px);',
    'background-size:136px 100%;animation:ocmBand 11s linear infinite;}',
    '#ocm-pop .bhead b{font-size:17px;font-weight:900;letter-spacing:-.3px;display:block;}',
    '#ocm-pop .bhead small{display:block;font-size:10px;font-weight:700;letter-spacing:1.5px;',
    'text-transform:uppercase;opacity:.86;}',
    '#ocm-pop .bbody{flex:1;overflow-y:auto;padding:4px 18px 22px;overscroll-behavior:contain;',
    '-webkit-overflow-scrolling:touch;}',
    '#ocm-pop .ocm-card{margin-top:14px;}',

    /* bouton « Ajouter un requis » et pastille de requis */
    '.ocm-addreq{flex:0 0 auto;border-radius:100px;padding:8px 16px;font-weight:800;font-size:12px;',
    'background:rgba(14,65,148,.08);color:var(--acc);border:1.5px dashed var(--acc) !important;',
    'transition:transform .16s var(--s),background .2s;white-space:nowrap;}',
    '.ocm-addreq:hover{background:rgba(14,65,148,.18);transform:translateY(-2px);}',
    '.ocm-addreq:active{transform:scale(.95);}',
    '.ocm-voy.rq{border-style:dashed !important;}',

    /* ── ANTI-SACCADE ──────────────────────────────────────────────────────
       Pendant le défilement, la décoration est mise en pause et les flous
       coupés : le compositeur n\'a plus qu\'à translater la couche, au lieu de
       redessiner dégradés et balayages à chaque frame. */
    '#ocm-bg{contain:strict;will-change:transform;}',
    '#ocm-ov.scrolling #ocm-bg i,',
    '#ocm-ov.scrolling .ocm-top::before,#ocm-ov.scrolling .ocm-top::after,',
    '#ocm-ov.scrolling .ocm-scan::before,#ocm-ov.scrolling .ocm-mode.on::after,',
    '#ocm-ov.scrolling .ocm-mode.arr:not(.on)::before,',
    '#ocm-ov.scrolling .ocm-k,#ocm-ov.scrolling .ocm-card,',
    '#ocm-pop.scrolling .ocm-k,#ocm-pop.scrolling .ocm-card,',
    '#ocm-pop.scrolling .bhead::after{animation-play-state:paused !important;}',
    '#ocm-ov.scrolling *,#ocm-pop.scrolling *{backdrop-filter:none !important;',
    '-webkit-backdrop-filter:none !important;}',
    '#ocm-ov.scrolling .ocm-k,#ocm-ov.scrolling .ocm-card,',
    '#ocm-pop.scrolling .ocm-k{transition:none !important;}',
    '.ocm-main{overscroll-behavior:contain;}',

    /* ── PANNEAU HISTORIQUE ────────────────────────────────────────────── */
    '#ocm-hist{position:fixed;inset:0;z-index:100066;display:none;align-items:center;',
    'justify-content:center;padding:18px;background:rgba(4,18,50,.52);}',
    '#ocm-hist.on{display:flex;animation:ocmFade .22s var(--e);}',
    '#ocm-hist .bulle{width:100%;max-width:1080px;max-height:92vh;display:flex;',
    'flex-direction:column;background:#F4F7FC;border-radius:22px;overflow:hidden;',
    'border:2px solid #0E4194;box-shadow:0 32px 80px rgba(0,20,60,.46);',
    'animation:ocmBulle .34s cubic-bezier(.34,1.5,.5,1) both;',
    "font-family:'DM Sans','Segoe UI',-apple-system,BlinkMacSystemFont,sans-serif;color:#0E2A5C;",
    '--j:#FDDD09;--v:#009841;--r:#E51932;--acc:#0E4194;--acc2:#134FB4;',
    '--line:rgba(14,65,148,.12);--muted:rgba(14,65,148,.6);--card:#FFFFFF;',
    '--sh:0 6px 20px rgba(14,65,148,.09);--sh2:0 16px 38px rgba(14,65,148,.18);',
    '--e:cubic-bezier(.22,.85,.28,1);--s:cubic-bezier(.34,1.5,.5,1);}',
    '#ocm-hist *{box-sizing:border-box;margin:0;}',
    '#ocm-hist button{font-family:inherit;cursor:pointer;border:0;}',
    '#ocm-hist .bhead{display:flex;align-items:center;gap:12px;padding:14px 20px;',
    'flex-wrap:wrap;background:linear-gradient(100deg,#0E4194,#134FB4);color:#fff;',
    'position:relative;overflow:hidden;flex-shrink:0;}',
    '#ocm-hist .bhead::after{content:"";position:absolute;left:0;right:0;bottom:0;height:4px;',
    'background:repeating-linear-gradient(90deg,#0E4194 0 34px,#009841 34px 68px,',
    '#00A8A8 68px 102px,#E51932 102px 136px);background-size:136px 100%;',
    'animation:ocmBand 11s linear infinite;}',
    '#ocm-hist .bhead b{font-size:17px;font-weight:900;letter-spacing:-.3px;display:block;}',
    '#ocm-hist .bhead small{display:block;font-size:10px;font-weight:700;letter-spacing:1.5px;',
    'text-transform:uppercase;opacity:.86;}',
    '#ocm-hist .bbody{flex:1;overflow-y:auto;padding:14px 18px 24px;',
    'overscroll-behavior:contain;-webkit-overflow-scrolling:touch;}',
    '.ocm-hday{background:#fff;border:1px solid var(--line);border-radius:14px;',
    'box-shadow:var(--sh);margin-bottom:10px;overflow:hidden;',
    'animation:ocmRise .42s var(--e) both;animation-delay:var(--d,0ms);}',
    '.ocm-hhead{display:flex;align-items:center;gap:10px;padding:12px 14px;cursor:pointer;',
    'flex-wrap:wrap;transition:background .18s;}',
    '.ocm-hhead:hover{background:rgba(253,221,9,.12);}',
    '.ocm-hdate{font-weight:900;font-size:14px;letter-spacing:-.2px;min-width:104px;}',
    '.ocm-hsum{display:flex;gap:6px;flex-wrap:wrap;flex:1;}',
    '.ocm-htag{font-size:10.5px;font-weight:800;border-radius:100px;padding:3px 10px;',
    'background:rgba(14,65,148,.09);color:#0E4194;border:1px solid rgba(14,65,148,.18);}',
    '.ocm-htag.v{background:rgba(0,152,65,.10);color:#047a37;border-color:rgba(0,152,65,.24);}',
    '.ocm-htag.y{background:rgba(253,221,9,.22);color:#7a5b00;border-color:rgba(253,221,9,.5);}',
    '.ocm-hchev{font-size:12px;color:var(--muted);transition:transform .24s var(--e);}',
    '.ocm-hday.open .ocm-hchev{transform:rotate(90deg);}',
    '.ocm-hdet{display:none;padding:0 14px 14px;}',
    '.ocm-hday.open .ocm-hdet{display:block;animation:ocmRise .3s var(--e);}',
    '.ocm-hleg{border-top:1px solid var(--line);padding:10px 0 4px;}',
    '.ocm-hlegt{font-size:11px;font-weight:900;letter-spacing:1px;text-transform:uppercase;',
    'color:var(--acc);margin-bottom:6px;display:flex;align-items:center;gap:8px;}',
    '.ocm-hlegt.arr{color:var(--v);}',
    '.ocm-hline{display:flex;align-items:center;gap:8px;padding:5px 0;font-size:13px;',
    'font-weight:700;border-bottom:1px dashed var(--line);}',
    '.ocm-hline .nm{flex:1;}',
    '.ocm-hline .ct{font-size:10.5px;font-weight:800;color:var(--muted);min-width:54px;}',
    '.ocm-hline input{width:64px;background:#F7F9FD;border:1.5px solid var(--line);',
    'border-radius:8px;padding:5px 8px;font-family:inherit;font-weight:800;font-size:13px;',
    'color:#0E2A5C;text-align:center;}',
    '.ocm-hline input:focus{outline:0;border-color:var(--acc);}',
    '.ocm-hx{background:#FDECEE;color:var(--r);border:1px solid rgba(229,25,50,.26) !important;',
    'border-radius:8px;padding:4px 9px;font-weight:900;font-size:12px;}',
    '.ocm-hactions{display:flex;gap:8px;flex-wrap:wrap;margin-top:12px;}',
    '.ocm-hro{font-size:11px;font-weight:800;color:var(--muted);margin-top:10px;',
    'background:rgba(14,65,148,.06);border-radius:8px;padding:8px 11px;}',
    '#ocm-hist.scrolling .ocm-hday,#ocm-hist.scrolling .bhead::after{',
    'animation-play-state:paused !important;}',

    /* ── KPI « ENTRÉ DANS LE CAMION » — jaune / vert néon ──────────────── */
    '#ocm-flux .ocm-card{--neon1:#D8FF00;--neon2:#39FF14;',
    'border:2px solid rgba(150,255,60,.55);',
    'background:linear-gradient(170deg,#FBFFF0,#F2FFF4) !important;',
    'box-shadow:0 10px 30px rgba(90,220,40,.20),inset 0 0 0 1px rgba(255,255,255,.7);}',
    '#ocm-flux .ocm-card::after{content:"";position:absolute;left:0;right:0;top:0;height:3px;',
    'background:linear-gradient(90deg,var(--neon1),var(--neon2),var(--neon1));',
    'background-size:200% 100%;animation:ocmNeon 3.2s linear infinite;}',
    '@keyframes ocmNeon{from{background-position:0 0}to{background-position:200% 0}}',
    '#ocm-flux .ocm-h{color:#3C7A00;text-shadow:0 0 12px rgba(140,255,60,.55);}',
    '#ocm-flux .ocm-h::after{background:linear-gradient(90deg,var(--neon1),var(--neon2));',
    'height:3px;box-shadow:0 0 10px rgba(140,255,60,.6);}',
    '#ocm-pop #ocm-flux .ocm-card{border-color:rgba(120,255,90,.6);}',
    '#ocm-ov.scrolling #ocm-flux .ocm-card::after{animation-play-state:paused !important;}',
    ...
    '#ocm-pop{padding:0;}',
    '#ocm-pop .bulle{max-width:100%;max-height:100%;height:100%;border-radius:0;border:0;}',
    '#ocm-pop .bbody{padding-left:12px;padding-right:12px;}',
    '}',
  ...
  function buildShell() {
    ...
    ov.innerHTML = [
      ...
      '  <button class="ocm-tbtn" id="ocm-hist-open" type="button">🕘 Historique</button>',
      ...
      '<main class="ocm-main" id="ocm-main">',
      '  <div id="ocm-work">',
    ...
    /* Bulle Arrivée : la barre des trajets et la zone de travail y sont
       DÉPLACÉES à l'ouverture, puis remises en place à la fermeture. Une seule
       zone de saisie existe donc, avec ses écouteurs — rien n'est dupliqué. */
    var pop = document.createElement('div');
    pop.id = 'ocm-pop';
    pop.setAttribute('role', 'dialog');
    pop.setAttribute('aria-label', 'Arrivée camion');
    pop.setAttribute('aria-modal', 'true');
    pop.innerHTML = [
      '<div class="bulle">',
      '  <div class="bhead">',
      '    <div><b>⬇ ARRIVÉE CAMION</b><small>Déchargement · matériel vide</small></div>',
      '    <div style="flex:1"></div>',
      '    <button class="ocm-tbtn" id="ocm-pop-close" type="button">✕ Fermer</button>',
      '  </div>',
      '  <div id="ocm-pop-bar"></div>',
      '  <div class="bbody" id="ocm-pop-body"></div>',
      '</div>'
    ].join('');
    document.body.appendChild(pop);

    /* Panneau Historique — même grammaire de bulle, thème bleu. Lecture pour
       tous, modification et suppression réservées au profil administrateur. */
    var hist = document.createElement('div');
    hist.id = 'ocm-hist';
    hist.setAttribute('role', 'dialog');
    hist.setAttribute('aria-label', 'Historique des rotations');
    hist.innerHTML = [
      '<div class="bulle">',
      '  <div class="bhead">',
      '    <div><b>🕘 HISTORIQUE DES ROTATIONS</b><small id="ocm-hist-sub">PIC Gillot · camion</small></div>',
      '    <div style="flex:1"></div>',
      '    <button class="ocm-tbtn" id="ocm-hist-close" type="button">✕ Fermer</button>',
      '  </div>',
      '  <div class="bbody" id="ocm-hist-body"></div>',
      '</div>'
    ].join('');
  ...
  /* Marque l'hôte pendant le défilement, puis retire la marque 140 ms après
     le dernier événement. Écouteur passif : il ne bloque jamais le scroll. */
  function scrollGuard(scroller, host) {
    if (!scroller || !host) return;
    var t = null;
    scroller.addEventListener('scroll', function () {
      if (!host.classList.contains('scrolling')) host.classList.add('scrolling');
      if (t) clearTimeout(t);
      t = setTimeout(function () { host.classList.remove('scrolling'); }, 140);
    }, { passive: true });
  }
  ...
  function wire() {
    ...
    on('ocm-pop-close', 'click', function () { setMode('dep'); });
    on('ocm-hist-open', 'click', openHist);
    on('ocm-hist-close', 'click', closeHist);
    on('ocm-hist', 'click', function (e) { if (e.target === $('ocm-hist')) closeHist(); });
    scrollGuard($('ocm-hist-body'), $('ocm-hist'));
    wireHist();
    on('ocm-pop', 'click', function (e) { if (e.target === $('ocm-pop')) setMode('dep'); });

    /* Anti-saccade : pendant le défilement, la décoration passe en pause et
       les flous sont coupés. Sans ça, chaque frame redessine les dégradés. */
    scrollGuard($('ocm-main'), $('ocm-ov'));
    scrollGuard($('ocm-pop-body'), $('ocm-pop'));
    ...
    document.addEventListener('keydown', function (e) {
      ...
      if (e.key === 'Escape') {
        ...
        if (st.hist) { closeHist(); return; }
        if (st.pop) { setMode('dep'); return; }
        close();
      }
    });
  ...
  function validateLeg() {
    ...
    commit();
    flush();
    toast('✓ ' + trajetLabel(st.mode) + ' validé — ' + tot + ' contenants', 'ok');
    resetScan(false);
  }
  ...
  /* Bascule de mode. L'arrivée n'occupe plus l'écran : la barre des trajets
     et la zone de travail sont déplacées dans la bulle, puis ramenées. */
  function setMode(m) {
    if (st.mode === m) return;
    st.mode = m;
    var ov = $('ocm-ov'), pop = $('ocm-pop');
    var bar = $('ocm-voybar'), work = $('ocm-work');
    if (!ov || !pop || !bar || !work) return;

    if (m === 'arr') {
      $('ocm-pop-bar').appendChild(bar);
      $('ocm-pop-body').appendChild(work);
      pop.classList.add('on');
      st.pop = true;
    } else {
      ov.insertBefore(bar, $('ocm-main'));
      $('ocm-main').insertBefore(work, $('ocm-kpis'));
      pop.classList.remove('on');
      st.pop = false;
    }
    ov.classList.toggle('arr', m === 'arr');
    ...
    resetScan(false);
    render();
    setTimeout(function () { var c = $('ocm-sc-c'); if (c) { try { c.focus(); } catch (e) { } } }, 260);
  }

  function legQte(leg) { return leg ? (legPleins(leg) + legVides(leg) + courTotal(leg)) : 0; }

  function renderVoybar() {
    var bar = $('ocm-voybar'); if (!bar) return;
    var day = gDay(), m = st.mode, t = st.tgt[m];
    var h = '<span class="ocm-vlab">' + (m === 'dep' ? 'Départs' : 'Arrivées') + '</span>';

    for (var i = 1; i <= 6; i++) {
      var leg = day.voy[i] ? day.voy[i][m] : null;
      var q = legQte(leg);
      h += '<button class="ocm-voy' + (t.type === 'voy' && t.n === i ? ' on' : '') +
        '" type="button" data-voy="' + i + '">' +
        (m === 'dep' ? 'Départ ' : 'Arrivée ') + i +
        (q ? ' · ' + q : '') +
        (leg && leg.done ? '<span class="dot"></span>' : '') + '</button>';
    }
    reqsOf(day, m).forEach(function (r, k) {
      var q = legQte(r.leg);
      h += '<button class="ocm-voy rq' + (t.type === 'req' && t.i === r.i ? ' on' : '') +
        '" type="button" data-req="' + r.i + '">Requis ' + (k + 1) +
        (q ? ' · ' + q : '') +
        (r.leg.done ? '<span class="dot"></span>' : '') + '</button>';
    });
    h += '<button class="ocm-addreq" type="button" data-addreq="1">+ Ajouter un requis</button>';
    bar.innerHTML = h;

    Array.prototype.forEach.call(bar.querySelectorAll('button'), function (b) {
      b.addEventListener('click', function () {
        if (b.hasAttribute('data-addreq')) { addRequis(st.mode); return; }
        if (b.hasAttribute('data-req')) st.tgt[st.mode] = { type: 'req', i: num(b.getAttribute('data-req')) };
        else st.tgt[st.mode] = { type: 'voy', n: num(b.getAttribute('data-voy')) || 1 };
        st.lastOp = null;
        resetScan(false);
        render();
        var c = $('ocm-sc-c'); if (c) { try { c.focus(); } catch (e) { } }
  ...
  function renderFlux() {
    ...
    var title = st.mode === 'dep'
      ? 'Entré dans le camion — ' + trajetLabel('dep')
      : 'Reçu du camion — ' + trajetLabel('arr');
  ...
  function renderKPIs() {
    ...
    h += kCard('t-leg', trajetLabel(st.mode), legPleins(leg), 'contenants pleins', '#0E4194', [], 0);
  ...
  /* ═══════════════ 11 bis. HISTORIQUE DES ROTATIONS ═══════════════ */

  /* Profil administrateur : on suit la règle du projet — c'est le PROFIL
     choisi au code PIN qui tranche (olympe_profil.type === 'admin'), jamais
     le rôle du compte partagé, qui est commun à toute l'équipe. Voir §6. */
  function isAdminUser() {
    try { if (typeof global.isAdmin === 'function' && global.isAdmin()) return true; } catch (e) { }
    try {
      var p = JSON.parse(localStorage.getItem('olympe_profil') || '{}');
      if (p && (p.type === 'admin' || p.compte === 'pfa')) return true;
    } catch (e) { }
    return false;
  }

  function histLegLabel(m, kind, n) {
    var base = (m === 'dep' ? 'Départ' : 'Arrivée');
    return kind === 'req' ? ('Requis ' + base.toLowerCase() + ' ' + n) : (base + ' ' + n);
  }

  /* Trajets d'un jour, avec la référence qui permet de les retrouver */
  function histLegs(date, m) {
    var day = (DB.days || {})[date], out = [], i, rq = 0;
    if (!day) return out;
    if (day.voy) for (i = 1; i <= 6; i++) {
      if (day.voy[i] && day.voy[i][m]) {
        out.push({ leg: day.voy[i][m], ref: { t: 'voy', n: i, m: m }, lab: histLegLabel(m, 'voy', i) });
      }
    }
    (day.req || []).forEach(function (r, idx) {
      if (!r || r.kind !== m) return;
      rq++;
      out.push({ leg: r, ref: { t: 'req', i: idx, m: m }, lab: histLegLabel(m, 'req', rq) });
    });
    return out;
  }

  function legByRef(date, ref) {
    var day = (DB.days || {})[date];
    if (!day) return null;
    if (ref.t === 'req') return (day.req || [])[ref.i] || null;
    return (day.voy && day.voy[ref.n]) ? day.voy[ref.n][ref.m] : null;
  }

  function histTotals(date) {
    var d = { dep: 0, arr: 0, vide: 0, cour: 0, done: 0 };
    ['dep', 'arr'].forEach(function (m) {
      histLegs(date, m).forEach(function (o) {
        d[m] += legPleins(o.leg);
        d.vide += legVides(o.leg);
        d.cour += courTotal(o.leg);
        if (o.leg.done) d.done++;
      });
    });
    return d;
  }
  ...
  function renderHist() {
    var box = $('ocm-hist-body'); if (!box) return;
    var admin = isAdminUser();
    var sub = $('ocm-hist-sub');
    if (sub) sub.textContent = admin ? 'PIC Gillot · modification autorisée' : 'PIC Gillot · lecture seule';

    var dates = Object.keys(DB.days || {}).filter(function (d) {
      if (!/^\d{4}-\d{2}-\d{2}$/.test(d)) return false;
      var t = histTotals(d);
      return (t.dep + t.arr + t.vide + t.cour) > 0;
    }).sort().reverse();

    if (!dates.length) {
      box.innerHTML = '<div class="ocm-empty"><span class="big">🕘</span>' +
        'Aucune rotation enregistrée pour l\'instant.</div>';
      return;
    }

    var h = '', d = 0;
    dates.forEach(function (date) {
      var t = histTotals(date);
      h += '<div class="ocm-hday' + (date === st.histOpen ? ' open' : '') +
        '" data-date="' + date + '" style="--d:' + Math.min(d, 480) + 'ms">';
      h += '<div class="ocm-hhead" data-toggle="' + date + '" role="button" tabindex="0">' +
        '<span class="ocm-hchev">▸</span>' +
        '<span class="ocm-hdate">' + esc(fmtDate(date)) + '</span>' +
        '<span class="ocm-hsum">' +
        '<span class="ocm-htag">⬆ ' + t.dep + ' départ</span>' +
        '<span class="ocm-htag v">⬇ ' + t.arr + ' arrivée</span>' +
        (t.vide ? '<span class="ocm-htag y">▸ ' + t.vide + ' vide</span>' : '') +
        (t.cour ? '<span class="ocm-htag">✉ ' + t.cour + ' courrier</span>' : '') +
        '<span class="ocm-htag">' + t.done + ' validé(s)</span>' +
        '</span></div><div class="ocm-hdet">';

      ['dep', 'arr'].forEach(function (m) {
        histLegs(date, m).forEach(function (o) {
          var lines = legLines(o.leg), cour = [];
          var bak = courContent(o.leg, 'BAK'), ke7 = courContent(o.leg, 'KE7'), kub = kubcTot(o.leg);
          if (bak) cour.push('BAK · ' + bak);
          if (ke7) cour.push('KÉ7 · ' + ke7);
          if (kub) cour.push('KUB Courrier · ' + kub);
          if (!lines.length && !cour.length) return;

          var rf = encodeURIComponent(JSON.stringify(o.ref));
          h += '<div class="ocm-hleg"><div class="ocm-hlegt' + (m === 'arr' ? ' arr' : '') + '">' +
            esc(o.lab) + (o.leg.done ? ' · validé' : '') +
            (o.leg.h ? ' · ' + esc(o.leg.h) : '') +
            (o.leg.etabli ? ' · ' + esc(o.leg.etabli) : '') + '</div>';

          lines.forEach(function (l) {
            var key = (l.prod === '__VIDE__' ? 'V' : PROD().indexOf(l.prod)) + '-' + l.col;
            h += '<div class="ocm-hline">' +
              '<span class="nm" style="color:' + (l.prod === '__VIDE__' ? '#8a6d00' : pcol(l.prod)) + '">' +
              esc(l.prod === '__VIDE__' ? 'VIDE — matériel' : l.prod) + '</span>' +
              '<span class="ct">' + esc(l.col) + '</span>' +
              (admin
                ? '<input type="number" min="0" value="' + l.qte + '" data-date="' + date +
                  '" data-ref="' + rf + '" data-key="' + key + '" data-act="qte">' +
                  '<button class="ocm-hx" data-date="' + date + '" data-ref="' + rf +
                  '" data-key="' + key + '" data-act="delline" title="Supprimer la ligne">✕</button>'
                : '<b>' + l.qte + '</b>') + '</div>';
          });
          cour.forEach(function (c) {
            h += '<div class="ocm-hline"><span class="nm" style="color:#8B5CF6">✉ ' + esc(c) +
              '</span><span class="ct">courrier</span></div>';
          });
          h += '</div>';
        });
      });
      ...
      h += admin
        ? '<div class="ocm-hactions"><button class="ocm-b r" data-date="' + date +
          '" data-act="delday">🗑 Supprimer la journée</button></div>' +
          '<div class="ocm-hro">Le courrier monté ne se modifie pas ici : sa ventilation par ' +
          'contenant se corrige dans Hermès, onglet Camion.</div>'
        : '<div class="ocm-hro">Lecture seule. La modification est réservée au profil administrateur.</div>';
      h += '</div></div>';
      d += 40;
    });
    box.innerHTML = h;
  ...
  function histCommit() {
    ...
    flush();
    renderHist();
    renderVoybar(); renderModeCounts(); renderFlux(); renderKPIs();
  }
  ...
  function wireHist() {
    ...
    box.addEventListener('click', function (e) {
      ...
      if (tog) {
        var dt = tog.getAttribute('data-toggle');
        st.histOpen = (st.histOpen === dt) ? null : dt;
        renderHist();
        return;
      }
      var b = e.target.closest ? e.target.closest('[data-act]') : null;
      if (!b || !isAdminUser()) return;
      var act = b.getAttribute('data-act'), date = b.getAttribute('data-date');

      if (act === 'delline') {
        var leg = legByRef(date, JSON.parse(decodeURIComponent(b.getAttribute('data-ref'))));
        if (!leg) return;
        delete leg.grid[b.getAttribute('data-key')];
        histCommit();
        toast('Ligne supprimée', 'ok');
        return;
      }
      if (act === 'delday') {
        /* Deux temps : un clic arme, le second confirme. Pas de confirm()
           natif, qui bloque le fil et se fait avaler par les navigateurs. */
        if (st.histConfirm !== date) {
          st.histConfirm = date;
          b.textContent = '⚠ Confirmer la suppression du ' + fmtDate(date);
          b.className = 'ocm-b j';
          setTimeout(function () {
            if (st.histConfirm === date) { st.histConfirm = null; renderHist(); }
          }, 5000);
          return;
        }
        st.histConfirm = null;
        delete DB.days[date];
        if (st.histOpen === date) st.histOpen = null;
        histCommit();
        toast('Journée du ' + fmtDate(date) + ' supprimée', 'ok');
      }
    });

    box.addEventListener('change', function (e) {
      var i = e.target;
      if (!i || i.getAttribute('data-act') !== 'qte' || !isAdminUser()) return;
      var leg = legByRef(i.getAttribute('data-date'), JSON.parse(decodeURIComponent(i.getAttribute('data-ref'))));
      if (!leg) return;
      var k = i.getAttribute('data-key'), v = num(i.value);
      if (v > 0) leg.grid[k] = v; else delete leg.grid[k];
      histCommit();
  ...
  function openHist() {
    ...
    st.hist = true;
    renderHist();
    var h = $('ocm-hist'); if (h) h.classList.add('on');
  }
  function closeHist() {
    st.hist = false;
    st.histConfirm = null;
    var h = $('ocm-hist'); if (h) h.classList.remove('on');
    render();
  }
  ...
  function close() {
    /* La bulle doit être refermée AVANT de masquer l'overlay : sinon la zone
       de travail resterait dans le popup et serait invisible à la réouverture. */
    if (st.pop) setMode('dep');
    var hh = $('ocm-hist'); if (hh) hh.classList.remove('on');
    st.hist = false; st.histConfirm = null;
    st.open = false;
  ...
  global.OlympeCamionApp = {
    ...
    voyage: function (n) {
      n = num(n);
      if (n >= 1 && n <= 6) { st.tgt[st.mode] = { type: 'voy', n: n }; render(); }
      return st.tgt[st.mode];
    },
    requis: function () { addRequis(st.mode); },
    historique: openHist,
    fermerHistorique: closeHist,
    admin: isAdminUser,
    state: function () { return { mode: st.mode, pop: st.pop, tgt: st.tgt, date: st.date, open: st.open }; },
