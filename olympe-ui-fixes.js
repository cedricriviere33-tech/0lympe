...
(function (global) {
  ...
  function startExtras() {
    ...
    s.textContent = [HUBW_CSS, BG_CSS, ANA_CSS, KG_CSS, STARS_CSS].join('\n');
    doc.head.appendChild(s);
    startHubBg();
  }

  /* ════════════════════════════════════════════════════════════════════
     H. CIEL ÉTOILÉ DU THÈME NUIT
     ════════════════════════════════════════════════════════════════════ */

  /* Le thème La Poste a posé un fond opaque sur #hub : une couche placée
     derrière lui ne se voit plus. Les étoiles sont donc un ENFANT de #hub,
     en z-index 0 — au-dessus de son fond, sous les cartes (z-index 1). */
  var STARS_CSS = [
    '#olp-stars{position:absolute;inset:0;z-index:0;pointer-events:none;',
    '  opacity:0;transition:opacity .7s ease;contain:strict;}',
    'html body:not(.hub-day) #olp-stars{opacity:1;}',
    '#olp-stars i{position:absolute;inset:0;display:block;background-repeat:repeat;}',

    /* Trois nappes de densités et de vitesses différentes : la profondeur
       vient du décalage, pas d'un calcul par étoile. */
    '#olp-stars .s1{background-image:',
    '  radial-gradient(1.6px 1.6px at 24px 38px,rgba(255,255,255,.9),transparent 100%),',
    '  radial-gradient(1.2px 1.2px at 132px 92px,rgba(255,209,0,.75),transparent 100%),',
    '  radial-gradient(1.8px 1.8px at 208px 24px,rgba(255,255,255,.8),transparent 100%),',
    '  radial-gradient(1.1px 1.1px at 76px 190px,rgba(255,255,255,.6),transparent 100%);',
    '  background-size:260px 260px;animation:olpTw 4.2s ease-in-out infinite alternate;}',
    '#olp-stars .s2{background-image:',
    '  radial-gradient(1.1px 1.1px at 58px 120px,rgba(255,255,255,.55),transparent 100%),',
    '  radial-gradient(1.5px 1.5px at 172px 62px,rgba(255,209,0,.5),transparent 100%),',
    '  radial-gradient(1px 1px at 300px 148px,rgba(255,255,255,.45),transparent 100%);',
    '  background-size:340px 340px;animation:olpTw 6.6s ease-in-out infinite alternate-reverse;}',
    '#olp-stars .s3{background-image:',
    '  radial-gradient(2.2px 2.2px at 96px 46px,rgba(255,209,0,.55),transparent 100%),',
    '  radial-gradient(2px 2px at 254px 206px,rgba(0,168,168,.4),transparent 100%);',
    '  background-size:420px 420px;animation:olpTw 9s ease-in-out infinite alternate;}',
    '@keyframes olpTw{from{opacity:.42}to{opacity:1}}',

    /* Deux étoiles filantes, très espacées : une traînée qui traverse et
       s\'efface. Rien de plus, sinon ça devient un écran de veille. */
    '#olp-stars b{position:absolute;width:120px;height:2px;border-radius:2px;opacity:0;',
    '  background:linear-gradient(90deg,transparent,rgba(255,255,255,.95),rgba(255,209,0,.9));',
    '  filter:drop-shadow(0 0 6px rgba(255,255,255,.8));',
    '  animation:olpShoot var(--dur,9s) linear var(--del,0s) infinite;}',
    '#olp-stars b.f1{top:14%;left:-140px;--dur:11s;--del:2s;}',
    '#olp-stars b.f2{top:52%;left:-140px;--dur:17s;--del:9s;transform:scale(.8);}',
    '@keyframes olpShoot{',
    '  0%{opacity:0;transform:translate3d(0,0,0) rotate(14deg);}',
    '  4%{opacity:1;}',
    '  16%{opacity:0;transform:translate3d(115vw,26vh,0) rotate(14deg);}',
    '  100%{opacity:0;transform:translate3d(115vw,26vh,0) rotate(14deg);}}',

    /* Pendant le défilement et en mouvement réduit, tout se fige. */
    'body.olp-scrolling #olp-stars i,body.olp-scrolling #olp-stars b{',
    '  animation-play-state:paused !important;}',
    '@media (prefers-reduced-motion:reduce){',
    '  #olp-stars i,#olp-stars b{animation:none !important;}',
    '  #olp-stars b{display:none;}',
    '}'
  ].join('\n');

  function startStars() {
    var hub = $('hub');
    if (!hub || $('olp-stars')) return;
    var el = doc.createElement('div');
    el.id = 'olp-stars';
    el.setAttribute('aria-hidden', 'true');
    el.innerHTML = '<i class="s1"></i><i class="s2"></i><i class="s3"></i>' +
      '<b class="f1"></b><b class="f2"></b>';
    hub.insertBefore(el, hub.firstChild);
  }

  /* ════════════════════════════════════════════════════════════════════
     I. DEMANDE DE POSITION QUI REVIENT EN BOUCLE
     ════════════════════════════════════════════════════════════════════ */

  /* index2.html ne contient AUCUN appel à navigator.geolocation : la demande
     vient d'un script externe (heartbeat d'appareil, très probablement). Le
     garde est donc posé sur l'API du navigateur, pas sur l'appelant.

     Trois règles :
       - un seul appel réel par session ; les suivants relisent le cache ;
       - un refus est définitif pour la session : plus aucune demande ;
       - si la permission est déjà « denied », on ne déclenche jamais l'appel,
         donc plus de bulle.
     Aucun chemin n'est bloqué en silence : l'appelant reçoit toujours son
     succès ou son erreur. */
  function startGeoGuard() {
    var geo = global.navigator && global.navigator.geolocation;
    if (!geo || geo.__olpGuard) return;

    var cache = null, cacheAt = 0, denied = null, inflight = false, queue = [];
    var MAX_AGE = 10 * 60 * 1000;
    var origGet = geo.getCurrentPosition.bind(geo);
    var origWatch = geo.watchPosition ? geo.watchPosition.bind(geo) : null;

    /* Si le navigateur sait déjà que c'est refusé, on n'appelle jamais. */
    try {
      if (global.navigator.permissions && global.navigator.permissions.query) {
        global.navigator.permissions.query({ name: 'geolocation' }).then(function (st) {
          if (st.state === 'denied') denied = { code: 1, message: 'Position refusée' };
          st.onchange = function () {
            if (st.state === 'denied') denied = { code: 1, message: 'Position refusée' };
            else if (st.state === 'granted') denied = null;
          };
        }).catch(function () { });
      }
    } catch (e) { }

    function settle(err, pos) {
      var q = queue; queue = []; inflight = false;
      q.forEach(function (c) {
        try { if (err) { if (c.err) c.err(err); } else if (c.ok) c.ok(pos); } catch (e) { }
      });
    }

    geo.getCurrentPosition = function (ok, err, opt) {
      if (denied) { if (err) setTimeout(function () { err(denied); }, 0); return; }
      if (cache && (Date.now() - cacheAt) < MAX_AGE) {
        if (ok) setTimeout(function () { ok(cache); }, 0);
        return;
      }
      queue.push({ ok: ok, err: err });
      if (inflight) return;
      inflight = true;
      origGet(function (pos) {
        cache = pos; cacheAt = Date.now();
        settle(null, pos);
      }, function (e) {
        if (e && e.code === 1) denied = e;   /* refus : définitif pour la session */
        settle(e, null);
      }, opt);
    };

    /* watchPosition rouvrait la bulle à chaque relance : une position suffit. */
    if (origWatch) {
      geo.watchPosition = function (ok, err, opt) {
        geo.getCurrentPosition(ok, err, opt);
        return -1;
      };
  ...
  /* ════════════════════════════════════════════════════════════════════
     J. ESPACE MANAGER — CADRE ET RACCOURCIS
     ════════════════════════════════════════════════════════════════════ */

  /* La barre bleue au-dessus du Manager n'est pas dans manager.html : c'est
     .oah-bar de #olympeAppHost, le cadre commun à TOUTES les applications
     ouvertes par OlympeApps. On ne la supprime donc pas — on la masque pour
     la seule application qui a déjà ses propres boutons Accueil et Fermer.
     Hephaistos et les autres gardent leur cadre. */
  var MG_CSS = [
    '#olympeAppHost.oah-bare .oah-bar{display:none !important;}',
    '#olympeAppHost.oah-bare iframe{height:100% !important;}',

    /* Bouton de retour, posé dans le document du hub : il survit à la
       fermeture de l\'hôte, sinon il disparaîtrait avec le Manager. */
    '#olp-mgback{position:fixed;left:50%;transform:translateX(-50%);bottom:22px;z-index:100098;',
    '  display:none;align-items:center;gap:9px;border:0;cursor:pointer;',
    '  background:linear-gradient(100deg,#0E4194,#134FB4);color:#fff;border-radius:100px;',
    '  padding:12px 24px;font-weight:900;font-size:13.5px;letter-spacing:.3px;',
    "  font-family:'DM Sans','Segoe UI',-apple-system,sans-serif;",
    '  box-shadow:0 14px 36px rgba(14,65,148,.46);',
    '  animation:olpMgIn .34s cubic-bezier(.34,1.5,.5,1);}',
    '#olp-mgback.on{display:inline-flex;}',
    '#olp-mgback:active{transform:translateX(-50%) scale(.96);}',
    '@keyframes olpMgIn{from{opacity:0;transform:translate(-50%,18px)}',
    'to{opacity:1;transform:translateX(-50%)}}'
  ].join('\n');

  /* Cibles atteignables depuis le volet latéral du Manager. Chacune décrit
     comment l'ouvrir dans le hub, et si l'hôte doit être refermé avant. */
  var MG_CIBLES = {
    hermes: { ferme: true, go: function () { call('switchSection', 'hermes'); } },
    historique: { ferme: true, go: function () { call('olympeOpenHistorique'); } },
    dashboard: { ferme: true, go: function () { obj('OlympeDashboard', 'open'); } },
    sos: { ferme: false, go: function () { win('sos.html'); } },
    papirus: { ferme: false, go: function () { win('papirus.html'); } },
    radio: { ferme: true, go: function () { if (!obj('OlympeRadio', 'open')) call('openApp', 'radio'); } },
    rg: { ferme: false, go: function () { win('https://www.reunion.aeroport.fr/'); } }
  };

  function call(fn) {
    var a = [].slice.call(arguments, 1);
    try { if (typeof global[fn] === 'function') { global[fn].apply(global, a); return true; } } catch (e) { }
    return false;
  }
  function obj(name, fn) {
    try {
      var o = global[name];
      if (o && typeof o[fn] === 'function') { o[fn](); return true; }
    } catch (e) { }
    return false;
  }
  function win(url) { try { global.open(url, '_blank', 'noopener'); } catch (e) { } }
  ...
  function mgBackBtn() {
    ...
    if (!b) {
      ...
      b.id = 'olp-mgback';
      b.type = 'button';
      b.innerHTML = '\u2190 Retour sur l\'app Manager';
      b.addEventListener('click', function () {
  ...
  function openManager() {
    try {
      if (global.OlympeApps && global.OlympeApps.open) {
        global.OlympeApps.open('manager.html', 'Espace Manager', '\uD83D\uDCCA');
        return true;
      }
    } catch (e) { }
    win('manager.html');
  ...
  /* Le Manager n'appelle jamais le hub directement : il poste un message.
     Il reste ainsi utilisable seul, hors iframe, sans code mort. */
  function startManager() {
    ...
    /* Cadre nu pour le Manager uniquement. */
    try {
      if (global.OlympeApps && global.OlympeApps.open && !global.OlympeApps.open.__olpBare) {
        var orig = global.OlympeApps.open;
        var wrapped = function (url, title, ico) {
          var r = orig.apply(this, arguments);
          var host = $('olympeAppHost');
          if (host) host.classList.toggle('oah-bare', String(url || '').indexOf('manager.html') >= 0);
          return r;
        };
        wrapped.__olpBare = true;
        global.OlympeApps.open = wrapped;
      }
    } catch (e) { }

    global.addEventListener('message', function (e) {
      var d = e && e.data;
      if (!d || typeof d !== 'object' || d.type !== 'olympe-manager-open') return;
      var c = MG_CIBLES[d.cible];
      if (!c) return;
      if (c.ferme) {
        try { if (global.OlympeApps && global.OlympeApps.close) global.OlympeApps.close(); } catch (er) { }
        mgBackBtn().classList.add('on');
        setTimeout(c.go, 220);
      } else {
        c.go();                       /* nouvel onglet : le Manager reste ouvert */
      }
    });
  }
  ...
  /* Le garde de position doit être posé avant tout autre script : on ne
     passe pas par boot(), qui attend le DOM. */
  try { startGeoGuard(); } catch (e) { }
  ...
  function boot() {
    ...
    try { startStars(); } catch (e) { console.warn('[olympe-ui-fixes] etoiles:', e); }
    try { startManager(); } catch (e) { console.warn('[olympe-ui-fixes] manager:', e); }
  }
