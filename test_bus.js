...
const CIBLE = path.resolve(process.argv[2] || 'index2_bus.html');
...
(async () => {
  ...
  chk(['hermes-iframe', 'analytics', 'historique'].every(m => pose.modules.indexOf(m) >= 0),
    'les trois vues sont inscrites', JSON.stringify(pose.modules));
  ...
  const routage = await page.evaluate(async () => {
    ...
    window.dispatchEvent(new CustomEvent('olympe:remote',
      { detail: { scope: 'olympe_pensebete_v1', entry_id: 'x', by: 'autre' } }));
