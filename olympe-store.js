...
(function (global) {
  ...
  var SCOPES = [
    ...
    'olympe_ctoc_v1', 'olympe_camstock_v1', 'olympe_finposte_v1',  // Dashboard Gillot (saisies manuelles)
    'olympe_dash_msg_v1', 'olympe_jsa_obj_v1', 'olympe_dash_obj_v1',  // Dashboard Gillot — message équipe + objectif JSA + objectif graphique
    'olympe_tv_locked',  // Verrou TV admin (pfa) — booléen synchronisé tous postes
    'olympe_planning_v1',    // Planning équipe — corrections manuelles du cycle glissant
    'olympe_agenda_v1',      // Espace agent — événements et rappels personnels
    'olympe_pensebete_v1',   // Espace agent — post-it personnels
    /* ── Correctif 2026-09-17 ─────────────────────────────────────────────
       hermes_mrd_histo et hermes_uld_histo figuraient dans MANAGED_KEYS
       (donc sauvegardés) mais pas ici : ils restaient locaux à chaque poste
       et ne remontaient jamais. Conséquence directe : le Dashboard Gillot ne
       pouvait pas les lire depuis Supabase et retombait sur localStorage.
       Ils sont ajoutés ICI *et* dans CUMULATIVE — voir le commentaire là-bas.
       Structures : uld {cp,nv,tu,np,pk,st} · mrd {cp,vl,tu,pa,ag,fait}
       ──────────────────────────────────────────────────────────────────── */
  ...
  var ADAPTERS = {
    ...
    // Relevé MRD : une ligne par palette relevée → fusion fine PC ↔ mobile
    // (un poste qui ajoute des palettes n'écrase jamais celles d'un autre).
  ...
  function diff(scope) {
    ...
    // Scopes CUMULATIFS (historiques) : on n'y supprime jamais une entree.
    // Une cle absente du blob local signifie seulement que ce poste ne l'a pas
    // encore hydratee -- surtout pas qu'il faut la detruire pour tout le monde.
    // C'est ce tombstone qui faisait disparaitre les anciens jours d'Hermes.
    var CUMULATIVE = ['hermes_gillot_v4', 'olympe_histo_ppi', 'olympe_histo_sac',
      ...
      /* ── Correctif 2026-09-17, indissociable de l'ajout dans SCOPES ──────
         Ajouter une clé à SCOPES sans l'ajouter ici est le scénario exact de
         l'incident de juillet : au premier hydrate d'un poste qui n'a pas
         toutes les journées en cache, le différentiel génère un marqueur de
         suppression sur chaque journée absente, et l'historique est détruit
         pour tout le monde. Les deux listes bougent ensemble, toujours.
         ──────────────────────────────────────────────────────────────── */
      'hermes_uld_histo', 'hermes_mrd_histo'];
      ...
      DB.init = function (onReady) {
        global.OlympeAuth.requireSession('../index.html')
          .then(function (ctx) {
            ...
            if (!global.OlympeMFA) {
              // Fail-closed : en ligne, l'absence du module MFA est une anomalie
              // (script bloque, en echec, ou desactive) -> on REFUSE l'acces
              // au lieu de laisser passer. C'etait un fail-open : correctif securite.
              // Tolere uniquement en mode test local, ou OlympeMFA est absent par design.
              if (API.isTestMode && API.isTestMode()) return null;
            ...
            }).then(function (r) { try { startPresence(); } catch (e) {} return r; });
          })
  ...
  try { console.info('[olympe-store] presence + gate + suppression volontaire \u2013 build 2026-09-17-STORE1'); } catch (e) {}
