...
(function (global) {
  ...
  var _rtChannel = null;      // canal realtime unique (évite le double-subscribe)
  var _pollTimer = null;      // timer de secours (évite les intervalles multiples)
  function subscribe() {
    if (!SB) return;
    // si déjà souscrit dans cette instance, on ne refait rien
    if (_rtChannel) { if (!_pollTimer) _pollTimer = setInterval(loadFeed, 15000); return; }
    try {
      // nettoyage d'un éventuel canal périmé partageant le même client Supabase
      // (cas du dashboard monté 2× : parent + iframe partagent le client)
      try {
        (SB.getChannels ? SB.getChannels() : []).forEach(function (ch) {
          if (ch && ch.topic && ch.topic.indexOf('olympe-dm-feed') !== -1) { try { SB.removeChannel(ch); } catch (e) {} }
        });
      } catch (e) {}
      // nom UNIQUE par instance → jamais de collision "callbacks after subscribe()"
      var chName = 'olympe-dm-feed-' + Math.random().toString(36).slice(2, 9);
      _rtChannel = SB.channel(chName)
    ...
    // filet : rafraîchissement périodique (une seule fois)
    if (!_pollTimer) _pollTimer = setInterval(loadFeed, 15000);
  }
  ...
  function mount(opts) {
    ...
    _encrypt = false; // chiffrement désactivé : messages en clair, lisibles dans toutes les apps (dont le Messenger d'Olympe). Le déchiffrement des anciens 'enc1:' reste actif à l'affichage.
    injectCss(); buildDom();
